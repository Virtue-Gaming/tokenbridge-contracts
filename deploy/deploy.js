...
async function deployInvertedNativeToErc(){
  const deployHome = require('./src/inverted_native_to_erc/home')
  const deployForeign = require('./src/inverted_native_to_erc/foreign')
  const { homeBridge } = await deployHome()
  const { foreignBridge } = await deployForeign()
  console.log('\nDeployment has been completed.\n\n')
  console.log(
